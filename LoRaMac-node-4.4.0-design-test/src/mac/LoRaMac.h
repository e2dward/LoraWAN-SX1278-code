...
{
    /*!
     * Defines the size of the applicative payload which can be processed  定义可以处理的应用有效载荷的大小
     */
    uint8_t MaxPossiblePayload;
    /*!
     * The current payload size, dependent on the current datarate  当前有效负载大小，取决于当前数据速率
     */
...
/*!
 * LoRaMAC events structure   LoRaMAC事件结构
 * Used to notify upper layers of MAC events  用于通知MAC事件的上层
 */
...
/*!
 * LoRaMAC callback structure  LoRaMAC回调结构
 */
typedef struct sLoRaMacCallback
{
    /*!
     * \brief   Measures the battery level  测量电池电量 电池电量0：节点连接到外部 电源，
	   * 																								    电池电量1..254：其中1为最小值  254是最大值，255：节点不能 测量电池电量]
     *
...
/*!
 * \brief   LoRaMAC layer initialization  LoRaMAC层初始化
 *
 ...
 * 除了初始化LoRaMAC层之外，该函数还初始化MCPS和MLME服务的回调原语。 \ ref LoRaMacPrimitives_t的每个数据字段都必须设置为有效的回调函数。
 *primitives - 指向定义LoRaMAC事件函数的结构的指针。 请参阅\ ref LoRaMacPrimitives_t。
 *events - 指向定义LoRaMAC回调函数的结构的指针。 请参阅\ ref LoRaMacCallback_t。
 *region - 要启动的区域。
 retval LoRaMacStatus_t操作的状态。 可能的回报是：
 *回报是：
 * \ ref LORAMAC_STATUS_OK，
 * \ ref LORAMAC_STATUS_PARAMETER_INVALID，
 * \ ref LORAMAC_STATUS_REGION_NOT_SUPPORTED。
