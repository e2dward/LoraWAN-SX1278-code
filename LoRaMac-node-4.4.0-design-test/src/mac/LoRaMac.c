...
/*!
 * Structure to hold an MCPS indication data.//用于保存MCPS指示数据的结构
 */
static McpsIndication_t McpsIndication;

/*!
 * Structure to hold MCPS confirm data.用于保存MCPS的结构确认数据。
 */
static McpsConfirm_t McpsConfirm;

/*!
 * Structure to hold MLME confirm data. 保持MLME确认数据的结构。
 */
...
/*!
 * \brief Function to be executed on Radio Tx Done event  要在Radio Tx Done事件上执行的功能
 */
...
/*!
 * \brief LoRaMAC layer frame buffer initialization  LoRaMAC层帧缓冲区初始化 将 macHdr 和
 * macPayload的 fCtrl 、FPort、FRMPayload 都传递进去，完成整个 MAC 层的数据组帧
 *
 * \param [IN] macHdr      MAC header field
...
static void OnRadioTxDone( void )//发送完成就开始 RX1 和 RX2 延时
{
